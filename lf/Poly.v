...
Proof.
  (* FILL IN HERE *) 
  intros. induction l1. 
  -simpl. destruct l2. +reflexivity.  + simpl. rewrite <- app_assoc. simpl. reflexivity.
...
Definition filter_even_gt7 (l : list nat) : list nat:=
  (* REPLACE THIS LINE WITH ":= _your_definition_ ." *)
   filter (fun a=> 7<=?a) (filter (fun a => evenb a) l).
...
Proof.
  ...
  -reflexivity.
  -simpl.
    assert (H0: forall (M N: Type)(g:M->N)(lm:list M)(m:M), map g (lm++[m])=map g lm++[g m]).
    {intros. induction lm. -reflexivity.  -simpl. rewrite IHlm. reflexivity. }
    rewrite H0.
    rewrite IHl.
    reflexivity. 
Qed.   
...
Example fold_example4:
  fold (fun (n:nat)(b:bool) =>true ) [1;2;3] true = true.
Proof. reflexivity. Qed.
