...
Proof.
  (* FILL IN HERE *) 
  intros P Q H NQ. unfold not. unfold not in NQ. intros HP. 
...
Proof.
  (* FILL IN HERE *) 
  intros P. unfold not. intros [HP NP]. apply NP in HP. destruct HP.
Qed.
(** [] *)
...
Proof.
  (* FILL IN HERE *) 
  intros X P H. unfold not. intros H1. 
  destruct H1 as [x E].
  apply E. apply H.
Qed.
(** [] *)
...
Proof.
   (* FILL IN HERE *) 
   intros X P Q. split.
   - intros H0. destruct H0 as [x [E0|E1]].
    + left. exists x. apply E0. + right. exists x. apply E1.
   - intros [H1|H2]. 
    + destruct H1 as [x E]. exists x. left. apply E.
    + destruct H2 as [x E]. exists x. right. apply E.
Qed.
(** [] *)
...
Proof.
  ...
  - intros y. simpl. split.
   + intros []. 
   + intros [x [E1 E2]]. apply E2.
  - intros y. simpl. split.
   + intros [H1|H2].
    * exists h. split. apply H1. left. reflexivity.
    * apply IHl in H2. destruct H2 as [x [E1 E2]]. exists x. 
      split. apply E1. right. apply E2.
   + intros [x [H1 [H21|H22]]].
    * rewrite H21. left. apply H1.
    * right. apply IHl. exists x. split. apply H1. apply H22.
Qed.
(** [] *)
...
Proof.
  ...
  - induction l as [|h l].
   ...
   + simpl. intros [H1|H2].
    * left. left. apply H1.
    * apply or_assoc. right. apply IHl. apply H2.
  - induction l as [|h l].
   + simpl. intros [H1|H2]. * destruct H1. * apply H2.
   + simpl. rewrite <- or_assoc. intros [H1|H2].
    * left. apply H1. * right. apply IHl. apply H2.
Qed.
(** [] *)
...
Proof.
  (* FILL IN HERE *) 
  intros T P l. induction l as [|h l].
  - split. 
  ...
  - split.
   ...
   + simpl. intros [H1 H2] x [H31|H32]. 
    * rewrite <- IHl in H2. rewrite <- H31. apply H1.
    * rewrite <- IHl in H2. apply H2. apply H32.
Qed.
(** [] *)
...
(* FILL IN HERE *) 
Proof. 
  assert(H: forall(X:Type)(l1 l2:list X),rev_append l1 l2 = rev l1 ++ l2).
  ...
  unfold tr_rev.
  rewrite H.
  induction (rev x) as [|h l IHrev]. 
  - reflexivity. - simpl. rewrite IHrev. reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  - rewrite evenb_S. destruct (evenb n) eqn:E.
    + simpl. destruct IHn as [k IHn0]. exists k. rewrite IHn0. reflexivity.
    + simpl. destruct IHn as [k IHn0]. exists (k+1). rewrite IHn0.
      assert(forall m: nat, S(S (double m))=double (m+1)).
...
Proof.
  (* FILL IN HERE *) 
  intros x y. split.
  - intros H. unfold not. intros H1. rewrite H1 in H. rewrite <- eqb_refl in H. discriminate H.
  - unfold not. intros H. destruct (x=?y) eqn:E. 
    + rewrite <- eqb_eq in H. apply H in E. destruct E. + reflexivity.
Qed.
(** [] *)
...
Proof.
(* FILL IN HERE *)
  intros A eqb H0 l1. induction l1 as [|h1 l1' IHl1]. 
  - intros l2. destruct l2 as [|h2 l2']. 
    + simpl. split. * intros H1. reflexivity. * intros H1. reflexivity.
    + simpl. split. * intros H1. discriminate H1. * intros H1. discriminate H1.
  - intros l2. destruct l2 as [|h2 l2'].
    + simpl. split. * intros H1. discriminate H1. * intros H1. discriminate H1.
    + simpl. destruct (eqb h1 h2) eqn:E. 
      * split. 
        ** rewrite IHl1. intros H2. rewrite H0 in E. rewrite E. rewrite H2. reflexivity.
        ** intros H2. injection H2 as H21 H22. rewrite IHl1. apply  H22.
      * split.
        ** intros H2. discriminate H2.
        ** intros H2. injection H2 as H21 H22. rewrite <- H0 in H21. rewrite H21 in E. discriminate E.
Qed.
(** [] *)
...
Proof.
  ...
  - intros H. induction l as [|h l IHl].
    + reflexivity.
    + simpl. simpl in H. rewrite andb_true_iff in H. destruct H as [H1 H2]. split. 
      * apply H1. * apply IHl. apply H2.
  - intros H. induction l as [|h l IHl].
    + reflexivity.
    + simpl. simpl in H. destruct H as [H1 H2]. rewrite andb_true_iff. split. 
      * apply H1. * apply IHl. apply H2.
Qed.
...
Definition excluded_middle := forall P : Prop,
  P \/ ~ P.
Theorem try: excluded_middle. Proof. intros P. unfold not. right. intros H. Abort.
...
Proof.
  (* FILL IN HERE *) 
  unfold excluded_middle. intros H X P. unfold not. intros H1 x.
  assert(H2: P x \/ ~ P x). { apply H. }
  destruct H2 as [H21|H22].
  - apply H21.
  - exfalso. apply H1. exists x. apply H22.
Qed.
(** [] *)
...
Proof.
  ...
  intros H1 P. unfold not. intros H2. apply H1 with (False).
  intros H3. apply H2 in H3. destruct H3.
Qed.
