...
Print even.
Inductive MyEq:nat -> nat -> Prop:=
| MyEq_O:MyEq 0 0
| MyEq_S x: MyEq x x -> MyEq (S x)(S x)
.
Check MyEq_S.
Print and. Check and. Check conj. Check forall A B : Prop, A -> B -> A /\ B.
Check (and (1=1)).
...
Check nat -> (0=0).
Check (0=0) -> nat.
Theorem test: forall A B : Prop, A -> B -> A /\ B.
Proof. intros. apply (conj H H0). Show Proof. Qed.
Check (fun (A B : Prop) (H : A) (H0 : B) => conj H H0).
Definition testProof:forall P Q:Prop, P -> Q -> (P->Q):=
fun (P Q:Prop)(HP:P)(HQ:Q) => (fun _ => HQ).

Theorem testNatProof:forall x y:nat, x=y -> y=x.
Proof. intros. symmetry in H. apply H. Show Proof. Qed.
Check eq_sym. (*forall (A : Type) (x y : A), x = y -> y = x*)
(* have "a proof of" x=y, get "a proof of" y=x *)
Definition testNatProof':forall x y:nat, x=y -> y=x :=
fun (x y : nat) (H : x = y) => eq_sym H.

(* Give H2:P->Q an H1 whose class is P as a parameter, 
  get a Hyp whose class is Q. *)
...
(** Notice that we terminate the [Definition] with a [.] rather than
    ...
    like 
    a normally-defined function.  ([Qed]-defined objects are
...
Definition and_comm'_aux P Q (H : P /\ Q) : Q /\ P :=
  match H with
  | conj HP HQ => conj HQ HP  (*have a proof of P/\Q => get a proof of Q/\P*)
  end.
...
Definition conj_fact_tmp : forall P Q R, P /\ Q -> Q /\ R -> P /\ R.
intros P Q R. Show Proof.
intros H1 H2. Show Proof.
apply conj. Show Proof.
destruct H1 as [H11 H12]. Show Proof.
apply H11. Show Proof.
destruct H2 as [H21 H22]. Show Proof.
apply H22. Show Proof.
Defined.

Check and_comm'_aux. Check conj.
...
Definition or_comm' P Q (H: P \/ Q): Q \/ P
  (* REPLACE THIS LINE WITH ":= _your_definition_ ." *)
:= match H with
   |or_introl HP => or_intror HP  (*have a proof of P => get a proof of Q\/P*)
                                      (*or*)
   |or_intror HQ => or_introl HQ  (*have a proof of Q => get a proof of Q\/P*)
   end.

Definition or_comm''' : forall P Q, P \/ Q -> Q \/ P
  (* REPLACE THIS LINE WITH ":= _your_definition_ ." *)
.
intros P Q. Show Proof.
intros H. Show Proof.
destruct H as [HP|HQ]. Show Proof.
apply or_intror. Show Proof.
apply HP. Show Proof.
apply or_introl. Show Proof.
apply HQ.
Defined.
(** [] *)
...
Definition some_nat_is_even : exists n, even n :=
  ex_intro even 4 (ev_SS 2 (ev_SS 0 ev_0)).   (*P    , x , P    x *)
                                              (*even , 4 , even 4 *)
...
Definition ex_ev_Sn : ex (fun n => even (S n))
  (* REPLACE THIS LINE WITH ":= _your_definition_ ." *)
:= ex_intro (fun n : nat => even (S n)) 1 (ev_SS 0 ev_0).
...
Proof.
(* FILL IN HERE *)
  intros X x y H P. destruct H. intros H. apply H.
Qed.
(** [] *)
...
apply (fun (P Q:Prop)(H1:P)(H2:P->Q) => (H2 H1)).
(*Give H2:P->Q an H1 whose class is P as a parameter, 
  get a Hyp whose class is Q.*)
Show Proof.
Qed.
