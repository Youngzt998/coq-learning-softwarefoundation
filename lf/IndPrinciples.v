...
Check foo_ind.
Inductive foo1 (X Y:Type):Type:=
  |bar1 (x:X)
  |baz1 (y:Y)
  |quux1 (f1:nat -> foo1 X Y)(n:nat).
Check foo1_ind.
...
Definition mult_0_r''':forall n:nat,P_m0r' n:=
nat_ind 
  P_m0r (* the "P" in nat_ind *)
    eq_refl  (* a proof of "P 0" in nat_ind *)
             (* a proof of "forall n:nat,P n -> P (S n)" in nat_ind  *)
      (fun (n : nat) (IHn : P_m0r n) (*have an "n" and a Hyp. that "P n"*)
          => IHn).  (*get a proof of "P (S n)", since "S n*0" is just n*0  *)
...
Inductive is_nat: nat -> Prop:=
  |is_nat_0: is_nat 0
  |is_nat_1 n: is_nat n -> is_nat (S n).
Check is_nat_ind.

Inductive eqn: nat -> nat -> Prop:=
  |eqn_0: eqn 0 0
  |eqn_1 n: eqn n n -> eqn (S n)(S n).
Check eqn_ind.

Inductive eqn': nat -> nat -> Prop:=
  |eqn'_0: eqn' 0 0
  |eqn'_1 n m: eqn' n m -> eqn' (S n)(S m).
Check eqn'_ind.
...
Check forall n : nat, even n -> Prop.
...
Proof.
  apply even_ind.   (*The "P" in even_ind is even'*)
  - (* ev_0 *)
    apply even'_0.
  - (* ev_SS *)
    intros m Hm IH.
