...
Theorem f_equal1 : forall (A B : Type) (f: nat -> nat) (x y: nat),
  x = y -> (f x =?f y)= false -> true = false.
Proof. intros A B f x y eq1 eq2. rewrite <- eq2.  rewrite eq1. apply eqb_refl. Qed.
...
Proof. 
  ...
  -intros m H0. 
   destruct m . 
   +simpl in H0. discriminate H0.
   +simpl in H0. apply S_injective in H0.
    replace (n'+S n') with (S n'+n') in H0. 
    replace (m+S m) with (S m+m) in H0.
    simpl in H0. apply S_injective in H0.
    apply IHn in H0.
...
Proof.
  (* FILL IN HERE *) 
  intros n. induction n as [|n' IHn].
  -destruct m as [|m']. +reflexivity. +simpl. intros H'. discriminate H'.
  -destruct m as [|m']. 
...
Proof.
  ...
  induction n.
  -intros l H. 
   destruct l. +reflexivity. +simpl in H. discriminate.
  -intros l H. 
   destruct l. +reflexivity. +simpl in H. apply IHn. injection H as goal. apply goal.
Qed.
(** [] *)
...
Proof.
  ...
  -intros l1 l2 eq1. injection eq1 as eq11 eq12. rewrite <-eq11. reflexivity.
  -intros l1 l2 eq1. destruct h as [x1 y1]. simpl in eq1. 
   destruct (split l'). injection eq1 as eq11 eq12. 
   rewrite <- eq11. rewrite <- eq12. simpl.
...
Proof.
  ...
  -destruct (f true) eqn:E1. 
   +rewrite E1. apply E1.
   +destruct (f false) eqn:E11.
    *apply E1. * apply E11.
  -destruct (f false) eqn:E2.
   +destruct (f true) eqn:E21.
    *apply E21. *apply E2.
   +rewrite E2. apply E2. 
Qed.
(** [] *)
...
Proof.
  ...
  -intros m. destruct m eqn:E1. +reflexivity. +reflexivity. 
  -intros m. destruct m eqn:E1. +reflexivity. +simpl. rewrite IHn. reflexivity.
Qed. 
(** [] *)
...
Proof.
  ...
  assert(H:forall q:nat,(q=?q)=true). 
  {induction q. -reflexivity. -simpl. apply IHq. } 
  apply H.
Qed.
(** [] *)
...
(* FILL IN HERE *) 
  ...
  induction l as [|h l' IHl].
  -intros l1 l2 eq1. injection eq1 as eq11 eq12. rewrite <- eq11. reflexivity.
  -intros l1 l2 eq2. destruct h as [x y]. simpl in eq2. 
   destruct (split l'). injection eq2 as eq21 eq22. rewrite <-eq21. rewrite <-eq22. 
...
Proof.
  ...
  induction l as [|h l' IHl].
  -intros eq1. unfold filter in eq1. symmetry in eq1. discriminate eq1.
  -intros eq2. simpl in eq2.
   destruct (test h) eqn:E.
   +injection eq2 as eq21 eq22. rewrite <- eq21. apply E.
   +apply IHl in eq2. apply eq2.
Qed.
(** [] *)
