...
Proof.
  (* FILL IN HERE *) 
  intros n. induction n. - apply ev_0. - simpl.  apply ev_SS. apply IHn.
Qed.
(** [] *)
...
Proof.
  (* FILL IN HERE *) 
  intros n H. inversion H as [|n' H1 H2]. inversion H1 as [|n'' H3 H4]. apply H3.
Qed.
(** [] *)
...
Proof.
  (* FILL IN HERE *)
  intros H. inversion H as [|n H1 H2]. inversion H1 as [|n' H3 H4]. inversion H3.
Qed.
(** [] *)
...
(* FILL IN HERE *)
  | sub1 : forall (l:list nat), subseq l l
  | sub2 : forall (l l1 l2 l3:list nat), subseq l1 (l2++l3) -> subseq l1 (l2++l++l3).
...
Proof.
  (* FILL IN HERE *) 
  intros. rewrite <- app_nil_r. rewrite <- app_assoc. 
  apply sub2 with (l1:=l1) (l2:=l2)(l3:=[]).
...
Proof.
  ...
  intros. induction H0.
  - apply H.
  - pose proof IHsubseq H. apply sub2. apply H1.
Qed.
(** [] *)
...
Inductive reg_exp {T : Type} : Type :=
  ...
  | EmptyStr                                  (* {epsilon} *)
  | Char (t : T)                              (* {t} *)
  | App (r1 r2 : reg_exp)                     (* {xy| x\in r1, y\in r2} *)
  | Union (r1 r2 : reg_exp)                   (* r1 \cup r2 *)
  | Star (r : reg_exp).                       (* {x_0x_1...x_k | x_i \in r forall 0<=i<=k forall k>=0  } *)
...
Proof.
  intros T s re H.
  rewrite <- (app_nil_r _ s).   (* or [rewrite <- (app_nil_r _ s).] *)
  apply (MStarApp s [] re).
  - apply H.
  - apply MStar0.
Qed.
...
Proof.
  (* FILL IN HERE *) 
  intros T ss re H. induction ss as [|hs ss' IHss ].
  - simpl. apply MStar0.
  - simpl. apply (MStarApp).
    + simpl in H. apply H. left. reflexivity.
    + apply IHss. intros s' H'. simpl in H. apply H. right. apply H'.
Qed.
(** [] *)
...
Proof.
  ...
  - generalize dependent s1. induction s2 as [|h2 s2' IHs2].
    + intros s1 H. simpl in H. inversion H. reflexivity.
    + intros s1 H. simpl in H. destruct s1 as [|h1 s1'].
      * inversion H.
        assert(H': forall Ty (a b:list Ty),a++b=[] -> a=[]).
        { intros Ty a b H'1. destruct a. - reflexivity. - simpl in H'1. discriminate H'1. }
        apply H' in H1. rewrite H1 in H3. inversion H3.
      * inversion H. rewrite H1.
        assert(H': forall Ty (a:list Ty) ch,a=~ Char ch -> a = [ch]).
        { intros Ty a ch H'1. inversion H'1. reflexivity. }
        apply H' in H3.  rewrite H3 in H1. simpl in H1. injection H1 as H11 H12. 
        rewrite H11. rewrite <- H12. apply IHs2 in H4. rewrite H4. reflexivity.
  - generalize dependent s1. induction s2 as [|h2 s2' IHs2].
      + intros s1 H. rewrite H. apply MEmpty. 
      + intros s1 H. simpl. rewrite H. apply (MApp [h2]). 
...
Proof.
  ...
  - intros H. induction re.
    ...
    + simpl in H. SearchAbout andb. apply andb_true_iff in H as [H1 H2]. 
      apply IHre1 in H1.  apply IHre2 in H2. 
      destruct H1 as [s1 H1]. destruct H2 as [s2 H2].
      exists (s1++s2). apply MApp. * apply H1. * apply H2.
    + simpl in H. apply orb_true_iff in H as [H1|H2]. 
      * apply IHre1 in H1. destruct H1 as [s1 H1]. exists s1. apply MUnionL. apply H1.
      * apply IHre2 in H2. destruct H2 as [s2 H2]. exists s2. apply MUnionR. apply H2.
    + exists []. apply MStar0.
Qed.
(** [] *)
...
Proof.
  ...
  - discriminate Heqre'.              (*MEmpty*)
  - discriminate Heqre'.              (*MChar*)
  - discriminate Heqre'.              (*MApp*)
  - discriminate Heqre'.              (*MUnionL*)
  - discriminate Heqre'.              (*MUnionR*)
  - injection Heqre'. intros H.       (*MStar0*)
    exists []. split. 
    + reflexivity.
    + intros s' H1. destruct H1.
  - pose proof IHexp_match2 Heqre'. destruct H1 as [ss' [HL HR]].
    exists (s1::ss'). split.
    + simpl. rewrite HL. reflexivity.
    + intros s' H1. destruct H1. (*unfold In in H1.*)
      * rewrite <- H1. injection Heqre'. intros H2. rewrite <- H2. apply H.
      * apply HR. apply H1.
Qed.
...
Proof.
  ...
  (* FILL IN HERE *)
  - simpl. omega.
  - simpl. intros H1. destruct s1 as [|h1 s1'].
    + simpl. exists []. exists s2. exists []. split.
      * rewrite app_nil_r. reflexivity.
      * split. 
        { intros contra. rewrite contra in H1. simpl in H1.
          inversion H1. 
          inversion Hmatch1. rewrite <- H2 in H0.
   Admitted.
...
Proof.
  (* FILL IN HERE *) 
  intros P b H. split.
  - intros HP. destruct b. 
    + reflexivity. 
    + apply ReflectT in HP. inversion H as [|H1]. inversion HP as [H2|].
      apply H1 in H2. destruct H2.
  - intros Hb. inversion H as[H1 H2|H1 H2].
    + apply H1. + rewrite Hb in H2. discriminate H2.
Qed.
(** [] *)
...
Proof.
  ...
  - simpl. destruct (eqbP n h) as [H|H].
    ...
    + simpl. intros H1 [H2|H2].
      * symmetry in H2. apply H in H2. apply H2.
      * apply IHl in H1. apply H1 in H2. apply H2.
Qed.
(** [] *) 
...
| nostutter_1: forall (h:X), nostutter [h]
| nostutter_S: forall (h a:X)(l:list X), 
                nostutter (a::l) -> h<>a -> nostutter (h::a::l).
...
| in_0_0_0:  in_order_merge [] [] []
| in_l_l1_l2_h1 : forall (l l1 l2: list X)(h1:X),
                  in_order_merge l l1 l2 -> in_order_merge (h1::l) (h1::l1) l2
| in_l_l1_l2_h2 : forall (l l1 l2: list X)(h2:X),
                  in_order_merge l l1 l2 -> in_order_merge (h2::l) l1 (h2::l1).

Theorem in_order_merge_test:forall {X} (l l1 l2:list X)(test:X->bool), 
in_order_merge l l1 l2 -> 
(filter test  l1 = l1 ) -> (filter test l2 = [])-> (filter test l = l1).

Proof.
  intros X l l1 l2 test. generalize dependent l2. generalize dependent l1.
  induction l as [|h l' IHl].
  - intros l1 l2 H1 H2 H3. simpl. inversion H1. reflexivity.
  - intros l1 l2 H1 H2 H3. simpl. destruct (test h) eqn:E.
    + induction l1 as [|h1 l1' IHl1].
      * inversion H1. rewrite <- H5 in H3. simpl in H3. rewrite E in H3. discriminate H3.
...
(* FILL IN HERE 
    ...
    [] *)
Theorem longest_filter: forall  (l l1 : list nat)(test:nat -> bool), 
subseq l1 l -> length (filter test l1) <= length (filter test l).
Proof. Admitted.
...
| pal_0: pal []
| pal_1 x: pal [x]
| pal_S x l: pal l -> pal (x::l++[x]).

Lemma rev_iff_0: forall X (l1 l2:list X), l1=l2 <-> rev l1 = rev l2.
Proof. Admitted.

Lemma rev_back_0: forall X (l1:list X)(y:X), rev(l1++[y])= (y::rev l1).
Proof. Admitted.

Lemma pal_rev_lemma: forall X (x:X)(l:list X), pal (x::l++[x]) -> pal l.
Proof.
  intros X x l H. remember (x::l++[x]) as l'. induction H.
  - discriminate Heql'.
  - injection Heql'. intros H. SearchAbout rev. 
    apply rev_iff_0 in H. simpl in H.
    rewrite rev_back_0 in H. discriminate H.
  - injection Heql'. intros H' H''. 
    rewrite rev_iff_0 in H'. rewrite rev_back_0 in H'. rewrite rev_back_0 in H'. 
    injection H'. intros H'''.
    rewrite <- rev_iff_0 in H'''. rewrite H''' in H. intros. apply H.
Qed.
...
Proof.
  ...
  - simpl. apply pal_0.
  - simpl. rewrite app_assoc. apply pal_S . apply IHL.
Qed.
...
(* FILL IN HERE 
    ...
    [] *)
Theorem pal_rev_ref: forall X (l:list X),  l = rev l -> pal l.
Proof.
  intros X l H. induction l as [|h l' IHL].
  - apply pal_0.
  - simpl in H. destruct (rev l') as [|t' l''].
    + simpl in H. injection H. intros H1. rewrite H1. apply pal_1. 
    + injection H. intros H1 H2. rewrite H1. 
      rewrite <- H2 in IHL. rewrite <- H2 in H. rewrite  H1 in IHL. rewrite H1 in H.
Admitted.
...
:= match l1 with
  | [] => False
  | h::l1' => ~(In h l2) /\ disjoint l1' l2
  end.

Inductive disjoint' {X:Type}: list X -> list X -> Prop:=
| disjoint_0_1 x : disjoint' [] [x]
| disjoint_1_0 x : disjoint' [x] []
| disjoint_S1 x l1 l2: disjoint' l1 l2 -> ~(In x l2) -> disjoint' (x::l1) l2
| disjoint_S2 x l1 l2: disjoint' l1 l2 -> ~(In x l1) -> disjoint' l1 (x::l2).
...
| no_dup_0: NoDup []
| no_dup_1 x: NoDup [x]
| no_dup_S x l: ~(In x l) -> NoDup l -> NoDup (x::l).
...
Proof.
  (* FILL IN HERE *) 
  intros X x l H. induction l as [|h l' IHL].
  - destruct H.
  - simpl in H. destruct H as [H1|H2].
    + exists []. simpl. exists l'. rewrite H1. reflexivity.
    + apply IHL in H2. destruct H2 as [l1 [l2 H2]].
      rewrite H2.  exists (h::l1). exists l2. simpl. reflexivity.
Qed.
...
Inductive repeats {X:Type} : list X -> Prop :=
  (* FILL IN HERE *)
|repeat_plus x l:  In x l -> repeats (x::l)
|repeat_induc x l: repeats l -> repeats (x::l).
...
Proof.
  ...
  (* FILL IN HERE *) 
  - intros l2 exc H1 H2. unfold excluded_middle in exc. inversion H2.
  - intros l2 exc H1 H2. simpl in H1. simpl in H2. Admitted.
(* Do not modify the following line: *)
...
Proof.
  ...
  - intros H. inversion H. destruct s1.
    + simpl in H1. simpl. left. split. * apply H3. * apply H4.
    + simpl in H1. simpl. right. inversion H1. rewrite <- H6. exists s1. exists s2. 
      split.
      * reflexivity. * split. apply H3. apply H4.
  - intros [[H1 H2]| [s0 [s1 [H1 [H2 H3]]]] ]. 
    + replace (a::s) with ([]++ a ::s). apply MApp. apply H1. apply H2. reflexivity.
    + rewrite H1. replace (a::s0++s1) with ((a::s0)++s1).
      apply MApp. apply H2. apply H3. reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  - intros H. remember (Star re) as re'. remember (a::s) as s'. induction H. 
    ...
    + discriminate Heqre'.
    + discriminate Heqs'.
    + injection Heqre'. intros H1. destruct s1 as [|h1 s1'].
      * simpl in Heqs'. apply IHexp_match2 in Heqre'. apply Heqre'. apply Heqs'. 
      * injection Heqs'. intros H2 H3. exists s1'. exists s2.
        split. rewrite H2. reflexivity. 
        split. rewrite <- H3. rewrite <- H1. apply H.
        apply H0.
  - intros [s0 [s1 [H1[H2 H3]]]].
    rewrite H1. replace (a::s0++s1) with ((a::s0)++s1). 
    apply MStarApp. apply H2. apply H3. reflexivity.
Qed.
(** [] *)
...
Check refl_matches_eps. (* [m:reg_exp -> bool] was checked automatically... *)
...
:= match re with
  ...
  | EmptyStr => true
  | Char a => false
  | App a b => match_eps a && match_eps b
  | Union a b => match_eps a || match_eps b
  | Star a => true
  end.
(** [] *)
...
Lemma app_nil_nil_nil: forall X (a b:list X), a++b=[] -> a=[]/\ b = [].
Proof. 
  induction a.
  - intros b H. simpl in H. split. reflexivity. apply H.
...
Proof.
  ...
  - simpl. apply ReflectF. apply empty_is_empty.
  - simpl. apply ReflectT. apply MEmpty.
  - simpl. apply ReflectF. unfold not. intros H. inversion H.
  - simpl. destruct (match_eps re1). inversion IHre1. 
    + simpl. destruct (match_eps re2). 
      * inversion IHre2. apply ReflectT. rewrite <- (app_nil_r _ []). 
        apply MApp. apply H. apply H0.
      * inversion IHre2. apply ReflectF. unfold not in * . intros H2. apply H0.
        inversion H2. 
         rewrite H1.
        apply app_nil_nil_nil in H1. destruct H1. rewrite H7 in H6. apply H6.
    + simpl. apply ReflectF. unfold not. intros H.
      inversion IHre1. apply H0. inversion H.  
      rewrite H1. apply app_nil_nil_nil in H1. destruct H1. rewrite <- H1. apply H4.
  - simpl. destruct (match_eps re1) , (match_eps re2).
    + simpl. apply ReflectT. apply MUnionL. inversion IHre1. apply H.
    + simpl. apply ReflectT. apply MUnionL. inversion IHre1. apply H.
    + simpl. apply ReflectT. apply MUnionR. inversion IHre2. apply H.
...
Proof.
  (* FILL IN HERE *) 
  unfold refl_matches_eps. intros re. apply iff_reflect. split.
  - intros H. induction re.
    + inversion H.
    + reflexivity.
    + inversion H.
    + inversion H. simpl. 
      assert(H': s1++s2=[]->s1=[]/\s2=[]). { admit. }
      apply H' in H1. destruct H1 as [H11 H12].
      rewrite H11 in H3. rewrite H12 in H4. apply IHre1 in H3. apply IHre2 in H4. 
      rewrite H3. rewrite H4. reflexivity.
    + inversion H. 
      * simpl. apply IHre1 in H2. rewrite H2. reflexivity.
      * simpl. apply IHre2 in H1. rewrite H1.
        assert (H'': forall a:bool, a || true = true). { admit. }
        apply H''.
    + inversion H. 
      *
*)
...
:= match re with 
  | EmptySet => EmptySet
  | EmptyStr => EmptySet
  | Char x => match (nat_of_ascii x =? nat_of_ascii a) with
              | true => EmptyStr
              | false => EmptySet
              end
