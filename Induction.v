...
Proof.
  ...
  rewrite -> H1.
  assert (H2: n+p=p+n). {rewrite plus_comm. reflexivity. }
  rewrite -> H2.
  rewrite -> plus_assoc.
  reflexivity.
  Qed.
...
Proof.
  (* FILL IN HERE *) 
  assert(H1: forall p:nat, p*0=0).
  {induction p as [|p' IHp0]. - reflexivity. -simpl. rewrite -> IHp0. reflexivity.  } 
  ...
  -simpl. 
   ...
   assert (H2: forall r s, r+r*s=r*S s).
   {intros r s. induction r as [|r IHr0]. 
    -simpl. reflexivity. 
    -simpl. 
     assert(H21: r+(s+r*s)=s+(r+r*s)). {rewrite -> plus_swap. reflexivity. }   
     rewrite -> H21.
     rewrite -> IHr0.
     reflexivity. }
   rewrite -> H2.
...
Proof.
  ...
  -simpl. 
   assert(H1:forall b:bin, bin_to_nat(incr b) = S(bin_to_nat b)).
    {intros b. induction b. -simpl. reflexivity. -simpl. reflexivity. 
